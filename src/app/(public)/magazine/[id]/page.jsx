...
const BookId = ({ params }) => {
  ...
  const magazines = lastMagazine.length <= 3 ? lastMagazine.length : 4;
  const settings = {
    slidesToShow: 2 ,
    slidesToScroll: 1,
    cssEase: "linear",
  ...
  const getLastMagazines = async ()=>{
    ...
    setLastMagazines(response)
    return 
  }
  const filterMagazine =  lastMagazine.filter((mag)=> mag.id !== Number(params.id))
  const total = cart?.reduce((acc, currentValue) => acc + currentValue.price, 0) || 0;
 
  return (
    <section className="w-full min-h-screen h-full mt-16 py-10 px-4 mx-auto">
      <div
        className={`${
          showModal
 ...
 <h1 className="py-4 uppercase text-black font-bold">Artigos nesta Edição</h1>
 ...
 <div className="w-full h-full grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-3 py-5">
   {magazine?.article?.map((article, index) => (
     ...
     >
      ...
      <div className="w-[80%] py-20 mx-auto h-full px-4">
